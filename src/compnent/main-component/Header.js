...
export default function Header() {
    ...
    //const valueofsearch = useSelector(s => s.MyCurrencyNames);
    ...
    const crytosearchFunction = (e) => {
        ...
        // Check if valueofsearch is an array before filtering
        /*      if (Array.isArray(valueofsearch) && valueofsearch.length > 0) {
                const filteredSearchResults = valueofsearch.filter((item) =>
                    item.name.toLowerCase().includes(searchTerm.toLowerCase())
                );
                setSearching(filteredSearchResults);
        ...
        <div >
            <div className="w-full flex ">
                ...
                <div className="mx-6 rounded-lg  flex items-center mx-0.5">
                    ...
                    <input className="px-2" type="text" id="search" placeholder=" search Cryto" onChange={crytosearchFunction} value={search} autoFocus ref={searchInputRef} />
                        ...
                        <div className="overflow-scroll absolute top-20 left-0 right-0 bottom-10 opacity-95 bg-white w-80 h-30 border border-green-600 z-10">
                            {search.length > 0 ? (
                                searching.length > 0 ? (
                                    <ul>
                                        {searching.map((item) => (
                                            <li className="p-2" key={item.id} onClick={() => handleSearchResultSelect(item.id)}>{item.name}</li>
                                        ))}
                                    </ul>
                                ) : (
