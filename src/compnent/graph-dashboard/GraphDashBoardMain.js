...
export default function GraphDashBoardMain({ searchingData }) {
    ...
    // const [selectedOptionId, setselectedOption] = useState(variablenameshown);
    ...
    }, [currencyvariablevalue, dayscount, variablenameshown, searchingData]);

    const data = {
        labels: graphdata && graphdata.prices ? graphdata.prices.map(price => {
        ...
        datasets: [
            {
                label: 'Crypto Prices:',
                data: graphdata && graphdata.prices ? graphdata.prices.map(price => price[1]) : [],
    ...
    const options = {
        ...
        scales: {
            y: {
                display: true,
                position: 'left',
                min: graphdata && graphdata.prices ? Math.min(...graphdata.prices.map(price => price[1])) * 0.95 : undefined,
                max: graphdata && graphdata.prices ? Math.max(...graphdata.prices.map(price => price[1])) * 1.05 : undefined,
            },
            x: {
                position: 'bottom',
            }
